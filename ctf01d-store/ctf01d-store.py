...
def preapareCacheUrl(u, suffix):
    ...
    return "./.ctf01d-store/cache/" + u2 + "_" + suffix +".json"
...
def cmd_download():
    ...
    if download_url == '':
        ...
        exit(0)
    dirto = './.ctf01d-store/downloads/service_' + name_
    print("Download: \n\tURL: " + download_url + "\n\tDirectory:" + dirto + "\n")
...
def cmd_install():
    ...
    shutil.copytree('./.ctf01d-store/downloads/service_' + name_ + '/checker', dir_checker)
    # TODO checker.py to chmod +x

    dir_vulnbox = './vulnbox/' + name_
    if os.path.isdir(dir_vulnbox):
        shutil.rmtree(dir_vulnbox)
    shutil.copytree('./.ctf01d-store/downloads/service_' + name_ + '/service', dir_vulnbox)
...
def cmd_init():
    ...
    create_dirs.append("./.ctf01d-store")
    create_dirs.append("./.ctf01d-store/cache")
    create_dirs.append("./.ctf01d-store/downloads")
    ...
    if not os.path.isfile("./.ctf01d-store/urls"):
    ...
    with open('./docker_jury/build_docker.sh', 'w') as f:
        f.write("#!/bin/bash\n")
        f.write("\n")
        f.write("docker build --rm=true -t 'ctfgame-" + game_id + "/ctf01d:" + fhq_jury_ad_use_version + "' .\n")
    ...
    with open('./docker_jury/clean_docker.sh', 'w') as f:
        f.write("#!/bin/bash\n")
        f.write("\n")
        f.write("docker rmi -f 'ctfgame-" + game_id + "/ctf01d:" + fhq_jury_ad_use_version + "'\n")
    os.chmod('./docker_jury/clean_docker.sh', stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO)

    print("Prepare ./docker_jury/Dockerfile")
    with open('./docker_jury/Dockerfile', 'w') as f:
        f.write("FROM freehackquest/ctf01d:" + fhq_jury_ad_use_version + "\n")
        f.write("\n")
...
services:
  ...
  """ + game_id + """_jury:
    depends_on:
      - """ + mysql_dbhost + """
    image: ctfgame-""" + game_id + """/ctf01d:""" + fhq_jury_ad_use_version + """
    volumes:
      - "./jury.d:/usr/share/ctf01d/jury.d"
    ports:
...
if command == "version":
    ...
    print("Will be used version of ctf01d: " + fhq_jury_ad_use_version)
