...
if len(sys.argv) != 2:
    print("Usage " + sys.argv[0] + " [start/clean/stop]")
    exit(0)
...
if sys.argv[1] == "stop":
    ...
    for proc in psutil.process_iter():
        ...
        if proc.name() == 'python2' and 'service1_test.py run' in command:
            proc.kill()
    exit(0)

if sys.argv[1] == "start":
    os.system('cd ' + parent_dir + ' && python2 ./service1_test.py run > /dev/null &')
    exit(0)
