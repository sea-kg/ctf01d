...
def loadAndParseKeyValue(filename, separ='='):
    ret = {}
    with open(filename, mode='r') as os_release:
...
def checkOS():
    if os.path.exists("/etc/debian_version"):
        OS="debian"
        OS_RELEASE = loadAndParseKeyValue("/etc/os-release")
        if OS_RELEASE['ID'] == "debian" or OS_RELEASE['ID'] == "raspbian":
            VERSION_ID = int(OS_RELEASE['VERSION_ID'])
            if VERSION_ID < 9:
                print("Your version of Debian is not supported.")
                print("")
                print("However, if you're using Debian >= 9 or unstable/testing then you can continue, at your own risk.")
                print("")
                continueYesOrNo()
        elif OS_RELEASE['ID'] == "ubuntu":
            OS="ubuntu"
            VERSION_ID = OS_RELEASE['VERSION_ID']
            MAJOR_UBUNTU_VERSION = int(VERSION_ID.split('.')[0][:-1])
            if MAJOR_UBUNTU_VERSION < 16:
                print("Your version of Ubuntu is not supported.")
                print("")
                print("However, if you're using Ubuntu >= 16.04 or beta, then you can continue, at your own risk.")
                print("")
                continueYesOrNo()
    elif os.path.exists("/etc/system-release"):
        OS_RELEASE = loadAndParseKeyValue("/etc/os-release")
        if OS_RELEASE['ID'] == "fedora" or OS_RELEASE['ID_LIKE'] == "fedora":
            OS = "fedora"
        if OS_RELEASE['ID'] == "centos":
            OS="centos"
            VERSION_ID = int(OS_RELEASE['VERSION_ID'])
            if VERSION_ID != 7 and VERSION_ID != 8:
                print("Your version of CentOS is not supported.")
                print("")
                print("The script only support CentOS 7 and CentOS 8.")
                print("")
                sys.exit(1)
        if OS_RELEASE['ID'] == "ol":
            OS="oracle"
            VERSION_ID = int(OS_RELEASE['VERSION_ID'])
            if VERSION_ID != 8:
                print("Your version of Oracle Linux is not supported.")
                print("")
                print("The script only support Oracle Linux 8.")
                sys.exit(1)
        if OS_RELEASE['ID'] == "amzn":
            OS="amzn"
            if OS_RELEASE['VERSION_ID'] != "2":
                print("Your version of Amazon Linux is not supported.")
                print("")
                print("The script only support Amazon Linux 2.")
                print("")
                sys.exit(1)
    elif os.path.exists("/etc/arch-release"):
        OS="arch"
    else:
        print("Looks like you aren't running this installer on a Debian, Ubuntu, Fedora, CentOS, Amazon Linux 2, Oracle Linux 8 or Arch Linux system")
        sys.exit(1)
...
def newClient():
    ...
    print("The name must consist of alphanumeric character. It may also include an underscore or a dash.")

    CLIENT = ''
    while not re.match(r'^[a-zA-Z0-9_-]+$', CLIENT):
    ...
    PASS = ''
    while PASS not in ('1','2'):
        print("Select an option [1-2]: ", end='')
        PASS = input()[0]

    CLIENTEXISTS = load_pki_index()
    if CLIENT in CLIENTEXISTS:
        print("")
        ...
        if PASS == '1':
            os.system('./easyrsa build-client-full "' + CLIENT + '" nopass')
        else:
            print("You will be asked for the client password below")
            os.system('./easyrsa build-client-full "' + CLIENT + '"')
    ...
    # } >>"$homeDir/$CLIENT.ovpn"
    print("")
    print("The configuration file has been written to $homeDir/$CLIENT.ovpn.")
...
def revokeClient():
    ...
    # TODO
    print("TODO do with something /etc/openvpn/ipp.txt")
...
def removeUnbound():
    ...
    os.remove('/etc/unbound/openvpn.conf')

    REMOVE_UNBOUND = ''
    while REMOVE_UNBOUND not in ('y', 'n'):
        print("")
        print("If you were already using Unbound before installing OpenVPN, I removed the configuration related to OpenVPN.")
        print("Do you want to completely remove Unbound? [y/n]: ", end='')
        REMOVE_UNBOUND = input()[0]

    if REMOVE_UNBOUND == 'y':
        # Stop Unbound
        os.system('systemctl stop unbound')

        if OS in ('debian', 'ubuntu'):
            os.system('apt-get remove --purge -y unbound')
        elif OS == 'arch':
            os.system('pacman --noconfirm -R unbound')
        elif OS in ('centos', 'amzn', 'oracle'):
            os.system('yum remove -y unbound')
        elif OS == 'fedora':
            os.system('dnf remove -y unbound')
        os.system('rm -rf /etc/unbound/')
        print("")
        print("Unbound removed!")
    else:
        os.system('systemctl restart unbound')
        print("")
        print("Unbound wasn't removed.")

def removeOpenVPN():
    print("")
    print("Do you really want to remove OpenVPN? [y/n]: ", end='')
    REMOVE=input()[0]
    if REMOVE == 'y':
        
        # Get OpenVPN port from the configuration
        # CONF = loadAndParseKeyValue("/etc/openvpn/server.conf", separ=' ')
        # PORT = CONF['port']
        # PROTOCOL = CONF['proto']

        # Stop OpenVPN
        if OS in ('fedora', 'arch', 'centos', 'oracle'):
            os.system('systemctl disable openvpn-server@server')
            os.system('systemctl stop openvpn-server@server')
            # Remove customised service
            os.remove('/etc/systemd/system/openvpn-server@.service')
        elif OS == "ubuntu" and VERSION_ID == "16.04":
            os.system('systemctl disable openvpn')
            os.system('systemctl stop openvpn')
        else:
            os.system('systemctl disable openvpn@server')
            os.system('systemctl stop openvpn@server')
            # Remove customised service
            os.remove('/etc/systemd/system/openvpn@.service')

        # Remove the iptables rules related to the script
        os.system('systemctl stop iptables-openvpn')
        # Cleanup
        os.system('systemctl disable iptables-openvpn')
        os.remove('/etc/systemd/system/iptables-openvpn.service')
        os.system('systemctl daemon-reload')
        os.remove('/etc/iptables/add-openvpn-rules.sh')
        os.remove('/etc/iptables/rm-openvpn-rules.sh')
        ...
        if OS in ('debian', 'ubuntu'):
            os.system('apt-get remove --purge -y openvpn')
            if os.path.exists('/etc/apt/sources.list.d/openvpn.list'):
                os.remove('/etc/apt/sources.list.d/openvpn.list')
                os.system('apt-get update')
        elif OS == 'arch':
            os.system('pacman --noconfirm -R openvpn')
        elif OS in ('centos', 'amzn', 'oracle'):
            os.system('yum remove -y openvpn')
        elif OS == 'fedora':
            os.system('dnf remove -y openvpn')
        ...
        os.system('rm -f /etc/sysctl.d/99-openvpn.conf')
        os.system('rm -rf /var/log/openvpn')

        # Unbound
        if os.path.exists('/etc/unbound/openvpn.conf'):
            removeUnbound()
        print("")
        print("OpenVPN removed!")
    else:
        print("")
        print("Removal aborted!")

def manageMenu():
    print("Welcome to OpenVPN-install!")
    print("The git repository is available at: https://github.com/angristan/openvpn-install")
    print("")
    print("It looks like OpenVPN is already installed.")
    print("")
    print("What do you want to do?")
    print("   1) Add a new user")
    print("   2) Revoke existing user")
...
if os.path.exists("/etc/openvpn/server.conf") and AUTO_INSTALL != 'y':
    manageMenu()
else:
    installOpenVPN()